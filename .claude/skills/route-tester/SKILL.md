...
### Token Expiration Bites You Silently

**Keycloak tokens expire. Old tokens = mysterious 401s:**

```bash
# You tested 30 minutes ago, worked fine
node scripts/test-auth-route.js http://localhost:3002/api/endpoint

# Now: 401 Unauthorized
# Cause: Token expired (default 5-30 minutes)

# SOLUTION: Always regenerate token before testing
# The script gets a fresh token each time, but if you're
# reusing curl commands from earlier output, they're stale
```

### Mock Auth Only Works in Dev

**Mock auth headers are ignored in production:**

```bash
# WORKS in development:
NODE_ENV=development
curl -H "X-Mock-Auth: true" -H "X-Mock-User: admin" http://localhost:3002/api/route

# SILENTLY IGNORED in production:
NODE_ENV=production
curl -H "X-Mock-Auth: true" ...  # Returns 401, mock headers ignored

# DANGER: Testing with mock auth doesn't test real auth flow
# A route might work with mock but fail with real Keycloak tokens
```

### Route Prefixes Are Confusing

**Each service has different prefix patterns:**

```typescript
// WRONG: Assuming consistent prefixes
curl http://localhost:3002/api/workflow/start  // 404!

// REALITY: Check each service's app.ts
// blog-api: '/blog-api/api' prefix
// notifications: '/notifications/api' prefix
// auth: '/api' prefix

// CORRECT: Full URL with prefix
curl http://localhost:3000/blog-api/api/workflow/start
```

### Cookie Name Must Be Exact

**The auth system expects exactly `refresh_token`:**

```bash
# WRONG: Different cookie name
curl -b "token=eyJ..." http://localhost:3002/api/route  # 401!
curl -b "auth_token=eyJ..." http://localhost:3002/api/route  # 401!

# CORRECT: Must be refresh_token
curl -b "refresh_token=eyJ..." http://localhost:3002/api/route
```
...
```bash
# You run a test
node scripts/test-auth-route.js http://localhost:3002/api/workflow/start POST '{"workflowCode":"TEST"}'

# Database now has a WorkflowInstance record
# Run it again: might fail due to duplicate constraints
# Or: test data accumulates, affecting other tests

# SOLUTION: Clean up test data after testing
docker exec -i local-mysql mysql -u root -ppassword1 blog_dev \
    -e "DELETE FROM WorkflowInstance WHERE workflowCode = 'TEST';"
```

### Keycloak Must Be Running

**No Keycloak = no tokens = no testing:**

```bash
# Script hangs or fails with connection error
node scripts/test-auth-route.js http://localhost:3002/api/route
...
curl http://localhost:8081/realms/yourRealm  # Should return JSON
```

### What These Patterns Don't Tell You

1. **Test user permissions** - `testuser` might not have all roles; tests might fail due to authorization, not authentication
2. **Database state** - Tests assume certain data exists; fresh database = different results
3. **Service dependencies** - Route might call other services; all must be running
4. **Request timeouts** - Default timeout might be too short for slow operations
5. **Error messages** - 500 errors don't show details; check `pm2 logs` for actual error
6. **SSL/TLS** - Production uses HTTPS; local testing uses HTTP; don't mix
